...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if(empty()){
    ...
    for(Node* n = head_; n!=tail_; n= n->next){
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // and an int.)
  Node* leftListNode = left.head_;
  Node* rightListNode = right.head_;
    while (leftListNode != nullptr && rightListNode !=nullptr){
    if(leftListNode->data <= rightListNode->data){
      merged.pushBack(leftListNode->data);
      leftListNode= leftListNode->next;
    }
    else{
      merged.pushBack(rightListNode->data);
      rightListNode= rightListNode->next;
    }
  }
  if(leftListNode !=nullptr){
    while (leftListNode !=nullptr)
    {
      merged.pushBack(leftListNode->data);
      leftListNode= leftListNode->next;
    }
  } 
  else if(rightListNode != nullptr){
    while(rightListNode != nullptr){
      merged.pushBack(rightListNode->data);
      rightListNode= rightListNode->next;
      }
  }
